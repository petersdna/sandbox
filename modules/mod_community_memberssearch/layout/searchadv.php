...
modMemberSearch = {
    action: {
        ...
        addCriteria: function () {
            var criteria = "";
            var keynum = modMemberSearch.action.keynum;

            criteria += '<div id="modmembersearch_criteria' + keynum + '" class="joms-form__item">';
            criteria += '<span id="selectfield' + keynum + '" class="joms-select--wrapper">';
            criteria += '<select name="field' + keynum + '" id="field' + keynum + '" onchange="modMemberSearch.action.changeField(\'' + keynum + '\');" class="joms-select">';
            <?php
            ...
            criteria += '</select>';
            criteria += '</span>';
            criteria += '<span id="modmembersearch_selectcondition' + keynum + '" class="joms-select--wrapper">';
            criteria += '<select name="condition' + keynum + '" id="condition' + keynum + '" class="joms-select">';
            criteria += '<option value=""></option>';
            criteria += '</select>';
            criteria += '</span>';
            criteria += '<span id="modmembersearch_valueinput' + keynum + '">';
            criteria += '<input type="text" name="value' + keynum + '" id="value' + keynum + '" class="joms-input"/>';
            criteria += '</span>';
            criteria += '<span id="modmembersearch_valueinput' + keynum + '_2">';
            criteria += '</span>';
            criteria += '<span id="typeinput' + keynum + '" style="display:none;">';
            criteria += '<input type="hidden" name="fieldType' + keynum + '" id="modmembersearch_fieldType' + keynum + '" value="" class="joms-input"/>';
            criteria += '</span>';
            criteria += '<span id="removelink' + keynum + '" class="removelinkbtn">';
            criteria += '<a href="javascript:void(0);" onclick="modMemberSearch.action.removeCriteria(\'' + keynum + '\');" class="joms-button--neutral joms-button--smallest">';
            criteria += '<?php echo JText::_('COM_COMMUNITY_REMOVE');?></a>';
            criteria += '</span>';
            criteria += '</div>';

            var comma = '';
            if (joms.jQuery('#modmembersearch_key-list').val() != "") {
                var comma = ',';
            }
            joms.jQuery('#modmembersearch_key-list').val(joms.jQuery('#modmembersearch_key-list').val() + comma + keynum);


            joms.jQuery('#modMemberSearchCriteriaContainer').append(criteria);
            modMemberSearch.action.changeField(keynum);
            modMemberSearch.action.keynum++;
        },
        removeCriteria: function (id) {
            var inputs = [];
            var _id, _id2;
            _id = joms.jQuery('#modmembersearch_key-list').val();
            _id2 = _id.split(',');
            ...
            joms.jQuery("#modmembersearch_criteria" + id).remove();
            joms.jQuery('#modmembersearch_key-list').val(inputs.join(','));
        },
        ...
        getListValue: function (id, fieldcode) {
            ...
            switch (fieldcode) {
                ...
                <?php } else {if ($field->type == 'list') { ?>
                    ...
                    list = '<span class="joms-select--wrapper"><select name="value' + id + '[]" id="value' + id + '" class="joms-select" multiple="multiple">';
                ...
                <?php } else { ?>
                    list = '<span class="joms-select--wrapper"><select name="value' + id + '" id="value' + id + '" class="joms-select"></span>';
                <?php
    ...
    ?>
        ...
        changeField: function (id) {
            ...
            type = modMemberSearch.action.getFieldType(value);
            ...
            condHTML = '<select class="joms-select" name="condition' + id + '" id="condition' + id + '" onchange="modMemberSearch.action.changeCondition(' + id + ');">';
            joms.jQuery(cond).each(function () {
                condHTML += '<option value="' + this + '">' + conditions[this] + '</option>';
            });
            condHTML += '</select>';

            joms.jQuery('#modmembersearch_selectcondition' + id).html(condHTML);
            modMemberSearch.action.changeCondition(id);
            modMemberSearch.action.calendar(type, id);
            if (listValue != 0) {
                joms.jQuery('#modmembersearch_valueinput' + id).html(listValue);
            }
        },
        addAltInputField: function (type, id) {
            ...
            joms.jQuery('#modmembersearch_valueinput' + id + '_2').html(inputField);
            if (cond === 'between') {
                if (type === 'birthdate' || type === 'date') {
        ...
        calendar: function (type, id) {
            ...
            if (type === 'birthdate' || type === 'date') {
                inputField += '<a href="javascript:" onclick="modMemberSearch.action.toggleAgeSearch(' + id + ', 1);" title="<?php echo addslashes(JString::trim(JText::_('COM_COMMUNITY_ADVSEARCH_AGE_TITLE'))); ?>"> <?php echo JText::_('COM_COMMUNITY_ADVSEARCH_DATE'); ?> </a>'
            ...
            joms.jQuery('#modmembersearch_valueinput' + id).html(inputField);
            if (type === 'birthdate' || type === 'date') {
        ...
        changeFieldType: function (type, id) {
            joms.jQuery('#modmembersearch_fieldType' + id).val(type);
        },
        changeCondition: function (id) {
            var type = joms.jQuery('#modmembersearch_fieldType' + id).val();
            this.addAltInputField(type, id);
        },
        toggleAgeSearch: function (id, mode) {
            var cond = joms.jQuery('#condition' + id).val();
            if (mode == 1) {
                inputField = '<a onclick="modMemberSearch.action.toggleAgeSearch(' + id + ',0);" href="javascript:void(0);" title="<?php echo addslashes(JString::trim(JText::_('COM_COMMUNITY_ADVSEARCH_DATE_TITLE'))); ?>"> <?php echo JText::_('COM_COMMUNITY_ADVSEARCH_AGE'); ?></a><input type="text" name="value' + id + '" id="value' + id + '" class="joms-input" value="" />';
                joms.jQuery('#modmembersearch_valueinput' + id).html(inputField);
                if (cond == "between") {
                    inputField = '<input type="text" name="value' + id + '_2" id="value' + id + '_2" class="joms-input" value="" />';
                    joms.jQuery('#modmembersearch_valueinput' + id + '_2').html(inputField);
                ...
                modMemberSearch.action.calendar('birthdate', id);
                modMemberSearch.action.addAltInputField('birthdate', id);
            }
        }
    }
};
...
<form name="jsform-search-advancesearch" class="js-form" action="<?php echo CRoute::_('index.php?option=com_community&view=search&task=advancesearch'); ?>"
      method="GET">
    <div id="modMemberSearchOptionContainer">
        <!-- Criteria Container begin -->
        <div id="modMemberSearchCriteriaContainer"></div>
        <!-- end: Criteria Container -->
        <div class="joms-form__group">
            <a class="joms-button--neutral joms-button--small" href="javascript:void(0);"
               onclick="modMemberSearch.action.addCriteria();">
                ...
                <script>
                    ...
                    joms_search_save = function () {
                        joms.api.searchSave({
                            ...
                            operator: joms.jQuery('#modMemberSearchOptionContainer').find('[name=operator]:checked').val(),
                            avatar_only: joms.jQuery('#modMemberSearchOptionContainer').find('[name=avatar]')[0].checked
                        });
                    };
                </script>
            <?php } ?>

            <input type="hidden" id="modmembersearch_key-list" name="key-list" value=""/>
        </div>
    </div>
