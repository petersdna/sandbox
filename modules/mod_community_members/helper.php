    ...
    class modCommunityMembers
    {
        function getMembersData($params)
        {
            ...
            switch ($filter) {
                case 0 :
                    // newest
                    $members = $model->getLatestMember($limit);
                    break;
                case 1 :
                    // popular
                    $members = $model->getPopularMember($limit);
                    break;
                case 2 :
                    // featured
                    $members = $model->getFeaturedMember($limit);
                    break;
                case 3 :
                    // friends
                ...
                case 4 :
                    // online
                    $members = $model->getLatestMember($limit, true);
                    break;
                default:
            ...
            foreach($members as $key=>$member){
                ...
                if(
                    ...
                    ($profileFilter && $profileTypeId && $member->_profile_id != $profileTypeId)
                ){
                    unset($members[$key]);
                }
            }
